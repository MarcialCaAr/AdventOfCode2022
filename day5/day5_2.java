...
public class day5_2 {
     ...
     * Remove the last quantity of characters from origin and insert them in destination from first to last
     */
    private static void moveCreate(int quantity, int origin, int destination){
        if(stacks[origin].size() < quantity)
            return;
        for(int i = quantity; i>0;i--){
            stacks[destination].add(stacks[origin].remove(stacks[origin].size()-i));
        }
    }
}